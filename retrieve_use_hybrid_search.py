...
def run_elasticsearch_query(query, index="pubmed25_with_vector"):
    es = Elasticsearch("http://109.105.34.64:9200", verify_certs=False, timeout=300)
...
# def create_query(query_string: str, query_vector = None, size=50):
#     return {"query": {"query_string": {"query": query_string}}, "size": size}
    ...
    if query_vector is not None:
        return {
            "query": {
                "bool": {
                    "should": [
                        {"query_string": {"query": query_string, "boost": 1}},
                        {
                            "knn": {
...
def process_question(
    ...
    try:
        ...
        query = create_query(query_string, query_vector)
        ...
        if len(relevant_articles) == 0:
            ...
            query = create_query(improved_query_string, query_vector)
            relevant_articles = run_elasticsearch_query(query)
            if len(relevant_articles) > 0:
...
def main():
    ...
    print(
        f"Model name: {model_name}, n-shot: {n_shot}, Time: {timedelta(seconds=time.time() - start_time)}"
    )

    model_name_pretty = model_name.split("/")[-1] if "/" in model_name else model_name
    output_filepath = f"/home/samsung/haoquan/bioasq2024-main/02_12B/Batch1/PhaseA/Results/{timestamp}_{model_name_pretty}_2024AB1-Fine-Tuned-{n_shot}-Shot.csv"
